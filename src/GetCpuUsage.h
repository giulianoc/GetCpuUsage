...
template <class T> class CPerfCounters
{
  public:
	CPerfCounters() {}
	~CPerfCounters() {}
  ...
  protected:
	class CBuffer
	{
	  public:
		...
		void Reset() { memset(m_pBuffer, NULL, m_Size); }
		operator LPBYTE() { return m_pBuffer; }
	...
	*/
	void QueryPerformanceData(PERF_DATA_BLOCK **pPerfData)
	{
		...
		*/
		...
		Buffer.Reset();
		while ((lRes = RegQueryValueEx(HKEY_PERFORMANCE_DATA, "Global", NULL, NULL, Buffer, &BufferSize)) == ERROR_MORE_DATA)
		{
	...
	*/
	T GetCounterValue(PPERF_OBJECT_TYPE pPerfObj, DWORD dwCounterIndex, LPCTSTR pInstanceName)
	{
		...
		else
		{
			...
			{
				if (stricmp(pInstanceName, (char *)((PBYTE)pPerfInst + pPerfInst->NameOffset)))
				{
